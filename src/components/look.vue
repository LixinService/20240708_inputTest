<template>
  ...
  <div :style="heigh" class="boxHeigh">
    <div style="width: 400px" :style="boxBorder" class="pb-8">
      <div
        v-if="!isXs"
        style="height: 65px; width: 100%; box-shadow: 0 1px 5px #000"
      ...
      <v-container>
        ...
        <div class="d-flex justify-center w-100 mx-auto">
          <v-window v-model="tab" style="width: 95%" :touch="false">
              ...
              <v-window-item value="two">
                <v-row>
                  <v-col cols="12" class="mt-8">
                    ...
                    <!-- 學號 -->
                    <v-text-field
                      v-if="role.value.value === UseRole.STUDENT"
                    ...
                    <v-text-field
                      ...
                      counter
                      :append-inner-icon="
                        visibleConfirm ? 'mdi-eye' : 'mdi-eye-off'
                      "
            ...
            <!-- 第三頁 -->
            <v-form
              @submit.prevent="submitThree"
              :disabled="form3.isSubmitting.value"
            >
...
const group = {
  items: [{ text: "社團", value: UseRole.CLUB }],
};
...
const schemaOne = yup.object({
  ...
  role: yup.number().required("請選擇註冊身分"),
});
...
const schemaThree = yup.object({
  // 學校名字
  schoolName: yup.string().required("學校必填").max(20, "學校長度不符"),
  // 學校地區
  schoolCity: yup
    .string()
    .required("學校縣市必填")
  ...
  // 檔案暱稱
  nickName: yup.string().max(6, "檔案暱稱長度不符"),
  // 出生日期
  BDAY: yup.string().test("isBADY", "出生日期必填", (value) => {
  ...
  studentGrade: yup
    ...
    .min(2, "年級長度不符")
    .max(2, "年級長度不符")
    .test("isStudentGrade", "年級必填", (value) => {
      if (role.value.value === UseRole.STUDENT && !value) {
        return false;
      }
      return true;
  ...
  // 社團幹部
  clubCoreMember: yup.object().shape({
...
// 日期格式轉換
const BDAYText = computed(() =>
  BDAY.value.value
    ? new Date(BDAY.value.value).toLocaleDateString().substring(0, 10)
    : ""
);
